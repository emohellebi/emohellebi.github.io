...
\sum_{i=1}^{n} \log\pi_0 & \text{if } y_i = 0 \\ 
\sum_{i=1}^{n} \log(1-\pi_0)\ + \ \sum_{i=1}^{n} \log(Gamma(\alpha, \beta)) & \text{if } y_i > 0 
\end{cases}
\end{equation*}$$
...
# png for Model 1
png("model1_training_plot.png", width = 800, height = 600)
plot1
dev.off()
...
model2 <- cito::dnn(
  ...
  epochs = 200, lr = 0.001, batchsize = 10,
  optimizer = "adam", validation = 0.2,
  verbose = F, plot = FALSE
  )
(plot2 <- cito:::visualize.training(
...
# png for Model 2
png("model2_training_plot.png", width = 800, height = 600)
plot2
dev.off()
...
model3 <- cito::dnn(
  ...
  epochs = 250, lr = 0.001, batchsize = 10,
  optimizer = "adam", validation = 0.2,
  verbose = F, plot = FALSE
  )
(plot3 <- cito:::visualize.training(
...
# png for Model 3
png("model3_training_plot.png", width = 800, height = 600)
plot3
dev.off()
...
# png for Model 4
png("model4_training_plot.png", width = 800, height = 600)
plot4
dev.off()
```
...
The Brier Score estimate was `Brier_Score`, which is a relatively small value, indicating that the model performs reasonably well in predicting whether it will rain or not.
