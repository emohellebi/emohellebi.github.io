...
In this first step of the project, we load all necesary [libraries](#lst-libraries-loaded) @lst-libraries-loaded, load and explore the weather dataset, prepare it for analysis according to the directions assigned to our group (Group 2), and then define a custom loss function for a semi-continuous gamma mixture model.

**1. Data Exploration and Preparation** 

We first load the dataset and [explore](#lst-load-dataset-and-explore-it) its structure and dimensions. @lst-load-dataset-and-explore-it
...
The month assigned to our group (Group 2) was June. We first subset the dataset to only include the [observations for June](#lst-Select-a-June-only-subset) @lst-Select-a-June-only-subset and then [convert](#lst-Kelvin-to-Celsius) @lst-Kelvin-to-Celsius all temperatures from Kelvin (K) to degrees Celsius (°C). We also [Set](#lst-Kelvin-to-Celsius) all precipitation values less than 0.01 to 0. @lst-Kelvin-to-Celsius



```{r Select a June only subset, include=FALSE}
...
```{r training/testing sets split, include=FALSE}
...
We split the subset dataset into [training and testing sets](#lst-training-testing-sets-split) @lst-training-testing-sets-split, we will use the data from 1961 to 2000 as the training set and the data from 1951 to 1960 and 2001 to 2014 as the testing set. The training set contains 30,000 observations, while the testing set contains 18,000 observations. 
...
We decided to fit multiple neural network [models](#lst-models-fit) @lst-models-fit for the semi-continuous gamma mixture model, combining different values for the number of hidden layers, the number of neurons in each layer, the epochs, and the batchsize. After experimenting with hundreds of these combinations we selected the 4 most reliable models and conducted some more runs to refine the experiment. [model1](#fig-model1-training-appendix) @fig-model1-training-appendix, [model2](#fig-model2-training-appendix) @fig-model2-training-appendix, [model3](#fig-model3-training-appendix) @fig-model3-training-appendix, [model4](#fig-model4-training-appendix) @fig-model4-training-appendix.

```{r Models 1, 2, 3, and 4 Fitting, include=FALSE}
# | fig-high: 12
# | fig-width: 6
...
The [best model](#lst-Best-Model) @lst-Best-Model was selected based on the combination of these two criteria, and it was used for the subsequent analysis in Part 3 and Part 4 of the project.


```{r pick best model, include=FALSE}
set.seed(007)
models <- list(
...
To assess the model numerically, we used the [Brier Score](#lst-Brier-Score) @lst-Brier-Score, defined by the formula
...
We proceeded to [calculate](#lst-Rainfall-mean-True-mean) @lst-Rainfall-mean-True-mean the mean rainfall from the $\alpha$ and $\beta$ parameters of the $Gamma$ distribution, per the following formula:

$$\mu = \frac{\alpha}{\beta}$$ These mean values were then [plotted](#fig-means-comp) @fig-means-comp against the actual observed rainfall values:
...
png("estimated_vs_true_means.png", width = 800, height = 600)
...
print(RMSE)
```
The resulting value for the [RMSE](#lst-RMSE) was `r RMSE` @lst-RMSE. Given the scale of the data, this value is not particularly low, however some reasonably outlying behavior was observed for some data points, which likely inflated some of the residuals, driving up the RMSE.
...
In order to answer the scientific question of whether the probability of no rainfall depends on temperature, we first [reload](#lst-load-again) @lst-load-again the intial weather dataset, [convert](#lst-Kelvin-to-Celsius-again) @lst-Kelvin-to-Celsius-again all temperature values from Kelvin (K) to degrees Celsius (°C), and subsetted the data for [June](#lst-June-subset-again) @lst-June-subset-again and [loction 1](#lst-loction1-subset-again) @lst-loction1-subset-again only. However, we did not subset the data for training and testing, as we wanted to use the entire dataset to answer this scientific question.


```{r load the initial weather dataset, include=FALSE}
...
```{r subset the data for June only, include=FALSE}
wholedata <- wholedata[month(wholedata$time) == 6, ]
```

```{r subset for location 1 only, include=FALSE}
wholedata <- wholedata[wholedata$loc == 1, ]
str(wholedata)

```

To visually assess how temperature influences $\pi_0$, we [plotted them against each other:](#fig-prob) @fig-prob
...
png("Probability_no_rainfall_vs_temperature.png", width = 800, height = 600)
...
Now, plotting the temperature against the 5%, 50% and 95% quantiles of the rainfall distribution results in the below plots: 

[Probability of no rainfall vs temperature](#lst-P-norain-vs-tmax) @lst-P-norain-vs-tmax
...
[Temperature vs 5% rainfall quantile](#fig-tmax-vs-05) @fig-tmax-vs-05


```{r, include=FALSE}

png("Temperature_vs_05_rainfall.png", width = 800, height = 600)
...
[Temperature vs 50% rainfall quantile](#fig-tmax-vs-50) @fig-tmax-vs-50


```{r, include=FALSE}

png("Temperature_vs_50_rainfall.png", width = 800, height = 600)
...
[Temperature vs 95% rainfall quantile](#fig-tmax-vs-95) @fig-tmax-vs-95


```{r, include=FALSE}

png("Temperature_vs_95_rainfall.png", width = 800, height = 600)
...
[Temperature vs 5% and 50% rainfall quantiles](#fig-tmax-vs-05_50) @fig-tmax-vs-05_50


```{r, include=FALSE}
plot(wholedata$tmax[z == 1], int2, main = "5%/50%", ylab = "Rainfall", xlab = "Temperature", col = "red")
lines(wholedata$tmax[z == 1], int1, col = "blue")
```


```{r, include=FALSE}

png("Temperature_vs_05_and_50_rainfall.png", width = 800, height = 600)
...
[Colored Temperature vs 95% rainfall quantile](#fig-col_tmax-vs-95) @fig-col_tmax-vs-95


```{r, include=FALSE}

plot(wholedata$tmax[z == 1], int3, main = "95%", ylab = "Rainfall", xlab = "Temperature", col = "green")
```


```{r, include=FALSE}

png("Colored_Temperature_vs_95_rainfall.png", width = 800, height = 600)
...
```{r}
#| label: fig-hist-plot
#| fig-cap: "Histogram for the probability of not raining"
#| echo: false

knitr::include_graphics("histogram_training_plot.png")
```
...
```{r}
#| label: fig-means-comp
#| fig-cap: "Estimated vs True Means Chart"
#| echo: false

knitr::include_graphics("estimated_vs_true_means.png")
```
...
```{r}
#| label: fig-prob
#| fig-cap: "Probability of no rainfall vs temperature"
#| echo: false

knitr::include_graphics("Probability_no_rainfall_vs_temperature.png")
```
...
```{r}
#| label: fig-tmax-vs-05
#| fig-cap: "Temperature vs 5% rainfall quantile."
#| echo: false

knitr::include_graphics("Temperature_vs_05_rainfall.png")
```
...
```{r}
#| label: fig-tmax-vs-50
#| fig-cap: "Temperature vs 50% rainfall quantile."
#| echo: false

knitr::include_graphics("Temperature_vs_50_rainfall.png")
```
...
```{r}
#| label: fig-tmax-vs-95
#| fig-cap: "Temperature vs 95% rainfall quantile."
#| echo: false

knitr::include_graphics("Temperature_vs_95_rainfall.png")
```
...
```{r}
#| label: fig-tmax-vs-05_50
#| fig-cap: "Temperature vs 5% and 50% rainfall quantiles"
#| echo: false

knitr::include_graphics("Temperature_vs_05_and_50_rainfall.png")
```
...
```{r}
#| label: fig-col_tmax-vs-95
#| fig-cap: "Colored Temperature vs 95% rainfall quantile."
#| echo: false

knitr::include_graphics("Colored_Temperature_vs_95_rainfall.png")
```
