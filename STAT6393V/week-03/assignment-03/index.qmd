...
subtitle: "Week 3 - Assignment 3: Model Comparison Australia Electricity Demand"
...
1.  The Normal distribution model from Assignment 2 (reuse results from Assignment 2)
2.  A quantile regression (QR) model which simultaneously estimates both quantiles
3.  The estimate of the quantiles from SPQR
...
1.  Have *at least* $0.2$ validation split in each case
2.  No need to have a separate test set
3.  For QR - use the training/validation loss as the metric for choosing your final model
4.  For SPQR - you can use the loss as the metric, but you can also use the `plotGOF` function to assess goodness of fit.
...
1.  Subset the data for December. Let's just compare the summer data.

2.  Get predictions for each model for each season. There should therefore be 2 different prediction dataframes. Save them as `qr_12` and `spqr_12`.
...
1.  Final architectures you chose for each model
2.  A table for the median, and a table for the 0.95 quantile. Each table should have the form:
...
m2 <- cito::dnn(cbind(Y,Y)~., data = dem_df,
        ...
        activation = "relu", verbose = TRUE, plot = TRUE)
```
...
```{r}
qr_12 <- predict(m2,dem12[,-1]) # QR approach
...
50% and 95% quantiles for SPQR:

```{r}
spqr_12_50 <- spqr_12[,1]
spqr_12_95 <- spqr_12[,2]
```
...
```{r}
# load the results from Assignment 2
...
m1 <- cito::dnn(
  ...
  verbose = F,
  validation = 0.2,
  custom_parameters = list(scale_par = 0.0),
  activation = "relu",
  optimizer = 'adam'
)

gaussian_12 <- predict(m1, dem12[,-1])
gaussian_12_50 <- c(gaussian_12)
...
gaussian_12_95 <- c(qnorm(0.95, mean = gaussian_12, sd = exp(m1$parameter$scale_par)))
```
